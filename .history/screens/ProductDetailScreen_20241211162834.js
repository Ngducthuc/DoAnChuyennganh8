...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const getProductDetails = async () => {
      try {
        console.log('Fetching details for product ID:', productId); // Log productId
    ...
    const checkIfFavorite = async () => {
      try {
        ...
        // Trích xuất tất cả `product_id` từ `data_product_id`
        const productIds = favorites.flatMap((item) =>
          Array.isArray(item.data_product_id) ? item.data_product_id.map(obj => obj.product_id) : []
        );
    
        console.log('Danh sách product_ids:', productIds);
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      await addFavoriteProductApi(token, productId); // Gọi API thêm sản phẩm yêu thích
