...
const CateSlide = ({ visible, onClose, onSelectCategory, categories }) => {
  ...
  return (
    <Animated.View style={[styles.slideContainer, { transform: [{ translateX: slideAnim }] }]}>
      ...
      <FlatList
        data={categories}
        keyExtractor={(item) => item.cartegory_id.toString()}
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.categoryItem}
            onPress={() => onSelectCategory(item)}
          >
            <Text style={styles.categoryText}>{item.cartegory_name}</Text>
          </TouchableOpacity>
        )}
...
const HomeScreen = () => {
  ...
  const [filteredProducts, setFilteredProducts] = useState([]); // Thêm state lọc sản phẩm
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      console.log("Danh mục nhận được từ API:", response);
      if (response && response.length > 0) {
        setCategories(response);
      } else {
        console.error("Không có danh mục trả về từ API");
      }
    } catch (error) {
  ...
  const fetchProductData = async () => {
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        setFilteredProducts(response); // Gán toàn bộ sản phẩm ban đầu vào danh sách lọc
      } else {
        console.error("Không có sản phẩm trả về");
      }
    } catch (error) {
  ...
  const fetchBannerImages = () => {
    // Tham chiếu tới ảnh trong thư mục assets
    setBannerImages([
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        await fetchCategoryData(); // Lấy danh mục
        await fetchProductData();  // Lấy sản phẩm
      } catch (error) {
  ...
  const handleSelectCategory = (category) => {
    ...
    if (category.cartegory_id === "All") {
      setFilteredProducts(products); // Hiển thị tất cả sản phẩm
    } else {
      const filtered = products.filter(
        (product) => product.cartegory_id === category.cartegory_id
      );
      setFilteredProducts(filtered); // Cập nhật danh sách sản phẩm lọc
    }
    setSliderVisible(false); // Đóng thanh danh mục
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.bannerSlider}>
        {bannerImages.map((banner) => (
          <Image key={banner.id} source={{ uri: banner.url }} style={styles.bannerImage} />
        ))}
      </ScrollView>
