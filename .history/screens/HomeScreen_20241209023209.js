...
const HomeScreen = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const response = await fetchProducts(token); // Gọi API
      console.log('API Response:', response);

      // Kiểm tra dữ liệu trả về
      if (response?.data) {
        const productsData = response.data.map((productGroup) => ({
          ...productGroup.data_product_id[0], // Lấy sản phẩm đầu tiên trong mảng `data_product_id`
          id: productGroup.id,
        }));

        setProducts(productsData); // Cập nhật state sản phẩm
      } else {
  ...
  useEffect(() => {
    fetchProductData(); // Gọi API khi component mount
  }, []);
