...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      // Kiểm tra nếu sản phẩm đã có trong danh sách yêu thích hay chưa
      if (isFavorite) {
        // Nếu sản phẩm đã yêu thích, xóa khỏi yêu thích
        await removeFavoriteProductApi(token, productId); // API xóa sản phẩm khỏi yêu thích
        setIsFavorite(false); // Cập nhật trạng thái yêu thích trong UI
        Toast.show({
          type: 'success',
        ...
        await addFavoriteProductApi(token, productId); // API thêm sản phẩm vào yêu thích
        setIsFavorite(true); // Cập nhật trạng thái yêu thích trong UI
        Toast.show({
          type: 'success',
      ...
      // Cập nhật lại danh sách yêu thích trong AsyncStorage và API
      const favorites = await fetchFavorites(token);
      const productIds = processFavorites(favorites);
      const isFav = productIds.includes(productId.toString());
      setIsFavorite(isFav); // Cập nhật trạng thái yêu thích sau khi thay đổi
