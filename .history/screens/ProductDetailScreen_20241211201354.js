...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  cconst [favorites, setFavorites] = useState([]); // Khởi tạo state favorites
  ...
  const handleAddFavorite = async (product) => {
    try {
      ...
      const isAlreadyFavorite = favorites.some(item => item.product_id === product.product_id);
      
      if (!isAlreadyFavorite) {
        // Thêm sản phẩm vào danh sách yêu thích
        const updatedFavorites = [...favorites, product];
  
        // Cập nhật lại danh sách yêu thích trong state
        setFavorites(updatedFavorites);
  
        // Cập nhật lại danh sách yêu thích trong AsyncStorage
