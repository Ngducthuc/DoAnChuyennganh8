    ...
    <Animated.View style={[styles.slideContainer, { transform: [{ translateX: slideAnim }] }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
             <Text style={styles.categoryText}>{item.cartegory_name}</Text>  {/* Hiển thị cartegory_name */}
          </TouchableOpacity>
        )}
...
const HomeScreen = () => {
  ...
  }, [token, selectedCategory]); // Khi token hoặc selectedCategory thay đổi, gọi lại API
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <CateSlide visible={sliderVisible} onClose={() => setSliderVisible(false)} onSelectCategory={handleSelectCategory} categories={categories} />
  ...
  renderItem={({ item }) => (
    <View style={styles.productItem}>
      <Image
        source={{
          uri: `https://doanchuyenganh.site/admin/uploads/${item.image.split('/').pop()}`,
        }}
        style={styles.productImage}
      />
      <Text style={styles.productName}>{item.name}</Text> {/* Bọc trong <Text> */}
      <Text style={styles.price}>{formatPrice(item.price_new)}</Text> {/* Bọc trong <Text> */}
    </View>
  )}
