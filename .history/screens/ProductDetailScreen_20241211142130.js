  ...
  useEffect(() => {
    const fetchProductDetailsAndFavoriteStatus = async () => {
      ...
      try {
        ...
        const favorites = await fetchFavorites(token); // Lấy danh sách yêu thích
        const isFav = favorites.some((item) => item.product_id === productId); // Kiểm tra trạng thái
        setIsFavorite(isFav);
        console.log(`Trạng thái yêu thích của sản phẩm (ID: ${productId}): ${isFav}`);
      } catch (error) {
        console.error('Error syncing favorite status:', error.message);
      } finally {
        setLoading(false);
      }
    };
