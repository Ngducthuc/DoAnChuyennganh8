...
const HomeScreen = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      // Gọi hàm fetchProducts với category và token
      const response = await fetchProducts('All', token);  // Gửi token cùng với category
      setProducts(response.products);  // Lưu sản phẩm vào state
