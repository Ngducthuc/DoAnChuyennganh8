...
const HomeScreen = () => {
  ...
  const fetchProductData = async () => {
    try {
      // Kiểm tra xem token có hợp lệ hay không trước khi gọi API
      if (!token) {
      ...
      // Gọi API với token
      const response = await fetchProducts('All', token); // Truyền token vào hàm fetchProducts
      console.log('API Response:', response); // Kiểm tra phản hồi API

      setProducts(response.products);  // Cập nhật sản phẩm vào state
      setLoading(false);  // Đổi trạng thái loading sau khi dữ liệu đã được tải
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetchProductData();  // Gọi API khi màn hình load và token đã có
    }
  }, [token]);  // Khi token thay đổi sẽ tự động gọi lại fetchProductData()
