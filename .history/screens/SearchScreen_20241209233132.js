...
const SearchScreen = () => {
  ...
  const handleProductClick = (product) => {
    setRecentSearches((prev) => {
      // Kiểm tra nếu sản phẩm đã tồn tại trong danh sách Recent Search
      const exists = prev.find((item) => item.product_id === product.product_id);
      if (exists) return prev; // Không thêm trùng lặp
      return [product, ...prev]; // Thêm vào đầu danh sách
    });
  };
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text style={styles.price}>{item.product_price_new}đ</Text>
            </TouchableOpacity>
          )}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  recentSearchImage: {
    ...
    height: 120, // Đảm bảo chiều cao đủ để hiển thị ảnh
    borderRadius: 10, // Bo tròn ảnh
    marginBottom: 10, // Khoảng cách giữa ảnh và văn bản
    borderWidth: 1,
    borderColor: '#ddd', // Thêm viền cho ảnh
    resizeMode: 'contain', // Đảm bảo ảnh không bị cắt và vẫn giữ tỷ lệ
  },
