...
const SignUpScreen = () => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      const response = await registerApi(username, email, password); // Gọi registerApi
      if (response.success) {
        // Hiển thị thông báo đăng ký thành công
        Alert.alert('Success', 'Account created successfully', [
          { 
            text: 'OK', 
            onPress: () => navigation.replace('HomeScreen')  // Thay navigate bằng replace để chuyển sang HomeScreen
          }
        ]);
      } else {
      ...
      if (error.message.includes('Email already registered')) {
        // Hiển thị thông báo lỗi nếu email đã đăng ký
        Alert.alert('Error', 'This email is already registered. Please use another email.');
      } else {
        // Hiển thị thông báo lỗi chung nếu gặp vấn đề khác
