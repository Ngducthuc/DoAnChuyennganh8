...
const HomeScreen = () => {
  ...
  // Lấy sản phẩm từ API
  const fetchProductData = async (category = 'All') => {
    try {
      const response = await fetchProducts(category);  // Sử dụng hàm fetchProducts từ api.js
      setProducts(response);  // Cập nhật dữ liệu sản phẩm vào state
      setLoading(false);
    } catch (error) {
  ...
  const loadMoreProducts = () => {
    // Giả sử bạn muốn tải thêm 5 sản phẩm mỗi lần
    const nextProducts = products.slice(0, products.length + 5);
    setProducts(nextProducts);
  };
  
  useEffect(() => {
    fetchProductData(selectedCategory);  // Fetch sản phẩm khi màn hình load hoặc danh mục thay đổi
  }, [selectedCategory]);
