...
const FavoritesScreen = () => {
  ...
  const loadFavorites = async () => {
    try {
      ...
      const favoritesData = (await fetchFavorites(token)) || []; // Mặc định là mảng rỗng
      setFavorites(favoritesData);
    } catch (error) {
      console.error('Lỗi khi tải danh sách yêu thích:', error.message);
      setFavorites([]); // Đặt trạng thái mặc định là mảng rỗng
    } finally {
      setLoading(false);
    }
  };
    ...
    const renderItem = ({ item }) => (
      <View style={styles.itemContainer}>
        ...
        <View style={styles.itemDetails}>
          ...
          <Text style={styles.itemPrice}>{item.product_price_new}đ</Text>
        </View>
        <TouchableOpacity>
