...
const SearchScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (token) {
        const results = await searchProducts(token, search); // Gọi API
        console.log('Search Results:', results);
        setProducts(results); // Cập nhật danh sách sản phẩm

        // Lưu lại từ khóa vào recentSearches
        setRecentSearches((prev) => [...new Set([search, ...prev])]);
      } else {
