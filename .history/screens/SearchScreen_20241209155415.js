...
const SearchScreen = () => {
  ...
  const handleSearch = async () => {
    if (!searchText.trim()) {
      return; // Nếu không có từ khóa tìm kiếm, không gọi API
    }
  
    setLoading(true);
    try {
      // Lấy token từ AsyncStorage trước khi gọi API
      const storedToken = await AsyncStorage.getItem('token');
      if (storedToken) {
        setToken(storedToken); // Cập nhật token vào state
        const response = await searchProducts(storedToken, searchText);
        console.log('Search Results:', response); // Kiểm tra kết quả trả về từ API
        setProducts(response.products || []); // Cập nhật danh sách sản phẩm
  
        // Lưu sản phẩm vào recentSearches nếu tìm thấy
        if (response.products && response.products.length > 0) {
          setRecentSearches(prevSearches => [
            ...new Set([...prevSearches, ...response.products.map(p => p.product_name)]),
          ]);
        }
      }
    } catch (error) {
  ...
  const testSearch = async () => {
    const token = 'Wj4djrxDsBYfWlqOEPkEdjY9u3LSc2';  // Token thực tế
    const searchText = 'ao';  // Từ khoá tìm kiếm
  
    try {
      const products = await searchProducts(token, searchText); // Gọi hàm searchProducts
  ...
  useEffect(() => {
    if (searchText.trim()) {
      handleSearch();  // Gọi tìm kiếm khi searchText thay đổi
    }
  }, [searchText]);  // Khi searchText thay đổi sẽ gọi lại hàm
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.searchBar}>
        ...
        <TextInput
          ...
          value={searchText}
          onChangeText={text => setSearchText(text)} // Cập nhật searchText
          />
