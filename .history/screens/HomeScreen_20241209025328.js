...
const HomeScreen = () => {
  ...
  const loadProducts = async () => {
    try {
      ...
      console.log('Token found:', token);  // Kiểm tra token đã lấy được chưa
      if (!token) {
      ...
      const data = await fetchProducts(token);  // Giả sử fetchProducts là hàm gọi API
      console.log('API Response:', data);  // Kiểm tra dữ liệu trả về

      if (data && data.products) {  // Kiểm tra xem API có trả về sản phẩm hay không
        setProducts(data.products);  // Lưu dữ liệu sản phẩm
      } else {
      ...
      setLoading(false);  // Tắt trạng thái loading khi API gọi xong
    }
  };

  // Gọi loadProducts khi component được mount
  useEffect(() => {
    loadProducts();
  }, []);
