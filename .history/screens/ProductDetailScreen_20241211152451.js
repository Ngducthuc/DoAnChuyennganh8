...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      // Gửi yêu cầu thêm/xóa sản phẩm yêu thích
      await addFavorite(token, productId);
      const updatedFavorites = await fetchFavorites(token); // Lấy lại danh sách yêu thích
      console.log('Danh sách yêu thích được cập nhật:', updatedFavorites);
  
      // Kiểm tra trạng thái yêu thích
      const isFav = updatedFavorites.some((item) => item.product_id === productId);
      setIsFavorite(isFav);
