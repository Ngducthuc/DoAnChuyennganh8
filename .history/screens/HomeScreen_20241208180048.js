...
const HomeScreen = () => {
  ...
  const fetchProductData = async () => {
    try {
      const response = await fetchProducts(selectedCategory);  // Lấy dữ liệu sản phẩm và token từ API
      setToken(response.token);  // Lưu token vào state
  ...
  useEffect(() => {
    fetchProductData();  // Fetch sản phẩm khi màn hình load hoặc danh mục thay đổi
  }, [selectedCategory]);
