...
const HomeScreen = () => {
  ...
  const fetchProducts = async (category = 'All') => {
    try {
      let url = 'https://doanchuyenganh.site/ModelApp/ProductApp.php'; // API để lấy tất cả sản phẩm
      if (category !== 'All') {
        url = `https://doanchuyenganh.site/ModelApp/ProductApp.php?category=${category}`; // Lấy sản phẩm theo danh mục
      }
  ...
  try {
    ...
    if (Array.isArray(response.data)) {
      const data = response.data.map(item => {
        ...
        item.image = `https://doanchuyenganh.site/admin/uploads/${item.image.split('/').pop()}`;
        return item;
      });
    ...
    setLoading(false);  // Đảm bảo cập nhật trạng thái loading sau khi có lỗi
  } finally {
    // Đảm bảo trạng thái loading luôn được cập nhật dù có lỗi hay không
    setLoading(false);
  }
};

  useEffect(() => {
    fetchProducts(selectedCategory); // Fetch sản phẩm khi màn hình load hoặc danh mục thay đổi
  }, [selectedCategory]);
