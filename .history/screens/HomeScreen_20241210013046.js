...
const CateSlide = ({ visible, onClose, onSelectCategory, categories }) => {
  ...
  return (
    <Animated.View style={[styles.slideContainer, { transform: [{ translateX: slideAnim }] }]}>
      ...
      <FlatList
        data={categories}
        keyExtractor={item => item.cartegory_id ? item.cartegory_id.toString() : item.id.toString()}
        renderItem={({ item }) => (
          <TouchableOpacity 
            style={styles.categoryItem}
            onPress={() => onSelectCategory(item)}
          >
            <Text style={styles.categoryText}>{item.cartegory_name || item.name}</Text>
          </TouchableOpacity>
        )}
...
const HomeScreen = () => {
  ...
  const formatPrice = (price) => {
    // Kiểm tra xem giá trị price có hợp lệ hay không
    if (price === undefined || price === null) {
      return 'Giá không hợp lệ';
    }
  
    // Chuyển price thành chuỗi và định dạng
  ...
  const handleSelectCategory = (category) => {
    setSelectedCategory(category.cartegory_name);
