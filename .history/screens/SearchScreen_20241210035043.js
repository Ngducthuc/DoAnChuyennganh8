...
const SearchScreen = () => {
  ...
  const saveRecentSearchLocally = async (product) => {
    try {
      ...
      console.log("Recent Searches Saved:", updatedSearches); // In ra để kiểm tra
    } catch (error) {
  ...
  const handleSearch = async () => {
    ...
    const handleProductClick = async (product) => {
      ...
      // Cập nhật danh sách recentSearches trong state
      setRecentSearches((prev) => {
        // Kiểm tra nếu prev là một mảng, nếu không thì khởi tạo là mảng rỗng
        const currentSearches = Array.isArray(prev) ? prev : [];
    
        // Kiểm tra nếu sản phẩm đã có trong danh sách, nếu có thì không thêm lại
        const exists = currentSearches.find((item) => item.product_id === product.product_id);
        if (exists) return currentSearches;
    
        // Thêm sản phẩm vào danh sách nếu chưa có
        return [product, ...currentSearches];
      });
    };
  ...
  // Hàm load lại lịch sử tìm kiếm từ AsyncStorage và API
  const loadRecentSearches = async () => {
    try {
      const storedSearches = await AsyncStorage.getItem('recentSearches');
      if (storedSearches) {
        const recentSearchesData = JSON.parse(storedSearches);
        console.log("Loaded Recent Searches:", recentSearchesData); // In ra để kiểm tra
    ...
    if (token) {
      try {
        const response = await axios.post('https://doanchuyenganh.site/ModelApp/RecentSeach.php', { token });
        console.log("Server Recent Searches:", response.data); // In ra để kiểm tra
...
// Gọi loadRecentSearches khi component mount
useEffect(() => {
  loadRecentSearches();
}, [token]);
