...
const CateSlide = ({ visible, onClose, onSelectCategory, categories }) => {
  ...
  return (
    <Animated.View style={[styles.slideContainer, { transform: [{ translateX: slideAnim }] }]}>
      ...
      <FlatList
        data={categories}
        keyExtractor={item => item.cartegory_id ? item.cartegory_id.toString() : item.id.toString()}  // Đảm bảo keyExtractor chính xác
        renderItem={({ item }) => (
          <TouchableOpacity 
          style={styles.categoryItem}
          onPress={() => onSelectCategory(item)}  // Truyền đối tượng danh mục khi chọn
          >
            <Text style={styles.categoryText}>{item.cartegory_name || item.name}</Text>  {/* Kiểm tra lại tên thuộc tính */}
          </TouchableOpacity>
        )}
...
const HomeScreen = () => {
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      const response = await fetchCategories(storedToken);
  ...
  const fetchProductData = async () => {
    try {
      ...
      // Kiểm tra xem response có phải là mảng và có dữ liệu hay không
      if (Array.isArray(response) && response.length > 0) {
        setProducts(response);  // Cập nhật sản phẩm vào state
      } else {
        console.error("Không có sản phẩm nào trong phản hồi");
        setProducts([]);  // Nếu không có sản phẩm, đảm bảo state sản phẩm là mảng rỗng
      }
    } catch (error) {
      console.error('Lỗi khi lấy sản phẩm:', error);
      // Có thể thông báo lỗi cho người dùng hoặc set state theo yêu cầu
    } finally {
      setLoading(false);  // Dù có thành công hay không, luôn dừng loading
    }
  };
  ...
  useEffect(() => {
    if (token) {
      fetchCategoryData();  // Lấy danh mục khi token có sẵn
      fetchProductData();  // Gọi API khi token có sẵn
    }
  }, [token]); // Khi token thay đổi, gọi lại API
  
  const handleSelectCategory = (category) => {
  setSelectedCategory(category.cartegory_name);  // Cập nhật tên danh mục đã chọn
  setSliderVisible(false);  // Đóng thanh trượt
};
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <CateSlide
          ...
          categories={categories} // Truyền state categories
    />
