...
const CateSlide = ({ visible, onClose, onSelectCategory, categories }) => {
  ...
  return (
    <Animated.View style={[styles.slideContainer, { transform: [{ translateX: slideAnim }] }]}>
      ...
      <FlatList
        data={categories}
        keyExtractor={(item) => item.cartegory_id.toString()} // Sử dụng cartegory_id làm key
        renderItem={({ item }) => (
        console.log("Danh mục được truyền vào CateSlide:", categories);
          <TouchableOpacity
            style={styles.categoryItem}
            onPress={() => onSelectCategory(item)} // Gửi danh mục khi chọn
          >
            <Text style={styles.categoryText}>{item.cartegory_name}</Text> {/* Hiển thị tên danh mục */}
          </TouchableOpacity>
        )}
...
const HomeScreen = () => {
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      const response = await fetchCategories(token); // Gọi API danh mục
      console.log("Danh mục nhận được từ API:", response); // Log dữ liệu trả về từ API
  ...
  useEffect(() => {
    console.log("Danh mục sau khi cập nhật:", categories); // Log danh mục khi state `categories` thay đổi
  }, [categories]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      />
        console.log("Dữ liệu danh mục được truyền vào CateSlide:", categories);
