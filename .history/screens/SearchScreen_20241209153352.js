...
const SearchScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (storedToken) {
        ...
        const response = await searchProducts(storedToken, searchText);  // Gọi API tìm kiếm
        ...
        if (response.products && response.products.length > 0) {
          setRecentSearches(prevSearches => [
            ...new Set([...prevSearches, ...response.products.map(p => p.name)]),  // Tránh trùng lặp
          ]);
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (token && searchText) {
      handleSearch();  // Gọi tìm kiếm khi token có giá trị và searchText thay đổi
    }
  }, [searchText, token]);  // Khi token hoặc searchText thay đổi sẽ gọi lại hàm
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={filterVisible ? styles.mutedContainer : null}>
        ...
        <FlatList
          data={recentSearches} // Dữ liệu mới từ các từ khóa tìm kiếm
          numColumns={3}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.productItem}>
              ...
              <Text style={styles.price}>{item.price_new}</Text>
            </View>
          )}
        />
      </View>
