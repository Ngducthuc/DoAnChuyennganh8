...
const SearchScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    // Lưu sản phẩm vào recent search
    if (token) {
      await saveRecentSearch(token, product);  // Lưu vào server
    } else {
    ...
    // Cập nhật danh sách recent searches trong state
    setRecentSearches((prev) => {
      const exists = prev.find((item) => item.product_id === product.product_id);
      if (exists) return prev; // Nếu sản phẩm đã có trong danh sách, không thêm lại
      return [product, ...prev]; // Thêm sản phẩm vào đầu danh sách
    });    
  };
