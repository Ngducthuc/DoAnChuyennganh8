...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      // Lấy lại danh sách yêu thích sau khi thêm
      const updatedFavorites = await fetchFavorites(token);
      const updatedProductIds = processFavorites(updatedFavorites);
  
      // Cập nhật trạng thái
      setIsFavorite(updatedProductIds.includes(productId.toString()));
