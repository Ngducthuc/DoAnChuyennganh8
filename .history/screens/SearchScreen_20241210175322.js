...
const SearchScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const response = await searchProducts(token, search); // Gọi API tìm kiếm
      if (Array.isArray(response)) {
        // Lọc dữ liệu theo từ khóa (nếu cần thiết)
  ...
  // Debounce hàm tìm kiếm để hạn chế gọi API khi nhập liệu liên tục
  ...
  const handleInputChange = (text) => {
    setSearch(text);
    debouncedSearch(text); // Gọi hàm debounce
  };

  
  const handleProductClick = async (product) => {
    // Lưu sản phẩm vào recent searches
    try {
      const storedSearches = await AsyncStorage.getItem('recentSearches');
      const recentSearches = storedSearches ? JSON.parse(storedSearches) : [];
      const updatedSearches = [product, ...recentSearches.filter(item => item.product_id !== product.product_id)];
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() => handleProductClick(item)} // Nhấn vào để xem chi tiết sản phẩm
          >
            <Image
