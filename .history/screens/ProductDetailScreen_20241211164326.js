...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      // Lấy lại danh sách yêu thích để đảm bảo trạng thái đồng bộ
      const updatedFavorites = await fetchFavorites(token);
  
      const productIds = processFavorites(updatedFavorites);
      const isFav = productIds.includes(productId.toString());  // So sánh với chuỗi
      setIsFavorite(isFav);
  
      Toast.show({
        type: 'success',
        text1: isFav ? 'Đã thêm vào mục yêu thích' : 'Đã xóa khỏi mục yêu thích',
      });
    } catch (error) {
