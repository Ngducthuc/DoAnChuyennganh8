...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const processFavorites = (favorites) => {
    const productIds = favorites.flatMap((item) => {
      if (Array.isArray(item.data_product_id) && item.data_product_id.length > 0) {
        return item.data_product_id.map((obj) => obj.product_id); // Trích xuất `product_id`
      }
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      await addFavoriteProductApi(token, productId);
  
      // Đổi trạng thái yêu thích ngay sau khi API gọi thành công
      const newFavoriteState = true; // Luôn đặt thành true khi đã thêm vào yêu thích
      setIsFavorite(newFavoriteState); // Cập nhật trạng thái yêu thích thành true
