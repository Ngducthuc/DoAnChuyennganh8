...
const HomeScreen = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const response = await fetchProducts(token); //goi api vs token
      console.log('API Response:', response); 

      if (response && response.products) {
        setProducts(response.products);  //update sp vao state
      } else {
  ...
  useEffect(() => {
    if (token) {
      fetchProductData(); //goi api khi token co san
    }
  }, [token]); //khi token thay doi thi tu dong goi lai fetchProductData()
