...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const [favorites, setFavorites] = useState([]); // Khởi tạo state favorites



  const processFavorites = (favorites) => {
    const productIds = favorites.flatMap((item) => {
      // Kiểm tra xem "data_product_id" có tồn tại và không rỗng không
      if (Array.isArray(item.data_product_id) && item.data_product_id.length > 0) {
        // Nếu "data_product_id" là một mảng chứa các đối tượng, ta cần kiểm tra chính xác
        return item.data_product_id.flatMap((obj) => {
          if (obj.product_id) {
            return obj.product_id.toString(); // Chuyển thành chuỗi để so sánh
          }
          return []; // Bỏ qua đối tượng nếu không có product_id
        });
      }
      return []; // Bỏ qua phần tử nếu không có data_product_id
    });
  ...
  useEffect(() => {
    ...
    checkFavoriteStatus();
  }, [productId]);  // Mỗi khi productId thay đổi, trạng thái yêu thích sẽ được kiểm tra lại
  ...
  const handleAddFavorite = async (product) => {
    try {
      ...
      // Lấy danh sách yêu thích hiện tại từ AsyncStorage
      ...
      favorites.push(product);  // Thêm sản phẩm hiện tại vào danh sách yêu thích
  
      // Cập nhật danh sách yêu thích vào AsyncStorage
      await AsyncStorage.setItem('favorites', JSON.stringify(favorites));
  
      // Cập nhật lại trạng thái yêu thích cho sản phẩm
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.productImageContainer}>
        ...
        />
        {/* Favorite Button */}
        <TouchableOpacity style={styles.favoriteIcon} onPress={handleAddFavorite}>
          <Icon
