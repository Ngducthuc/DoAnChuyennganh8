...
const ProductDetailScreen = ({ route, navigation }) => {
    ...
    const getProductDetails = async () => {
      ...
      const checkIfFavorite = async () => {
        try {
          ...
          const favorites = await fetchFavorites(token); // Gọi API lấy danh sách yêu thích
  ...
  const handleAddToFavorite = async () => {
    try {
      ...
      const response = await addFavorite(token, productId); // Gọi API thêm yêu thích
      if (response.success) {
        setIsFavorite(true); // Cập nhật trạng thái yêu thích
        Toast.show({
          type: 'success',
          text1: 'Thêm vào mục yêu thích thành công',
        });
      } else {
        Toast.show({
          type: 'error',
          text1: response.message || 'Thêm vào mục yêu thích thất bại',
        });
      }
    } catch (error) {
      ...
      Toast.show({
        type: 'error',
        text1: 'Lỗi khi thêm vào mục yêu thích',
      });
    }
  };
